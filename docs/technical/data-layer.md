...
same reason `Templating\`/`Settings\` did, and now sits directly in `Repositories\` the way
`Database\Abstract_Installer` sits directly in `Database\`. A split into a bare base plus an
entity-aware subclass was tried and reverted - every current repository is entity-backed,
so the split had no second consumer and existed only in case one shows up; add it back if
one actually does.
